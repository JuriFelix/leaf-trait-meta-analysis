...
# SR, PD, density = continuous variables
...
N_Bpn_ST<- rma.mv(yi,vi,data=ef, subset=(trait_category=="N" & species != "Betula pendula" ),method="REML",
                 random = list( ~ 1 | experiment, ~ 1 | ACC, ~1 | species, ~1 | EF),
               R = list(species = cor), control = list(optimizer = "optim"),  mods = ~ study_type)
...
phenolics_Bpn_N<- rma.mv(yi,vi,data=ef, subset=(trait_category=="phenolics" & species != "Betula pendula" ),method="REML",
                 random = list( ~ 1 | experiment, ~ 1 | ACC, ~1 | species, ~1 | EF),
               R = list(species = cor), control = list(optimizer = "optim"),  mods = ~ Nfixing)
...
phenolics_Bpn_LS<- rma.mv(yi,vi,data=ef, subset=(trait_category=="phenolics" & species != "Betula pendula" ),method="REML",
                 random = list( ~ 1 | experiment, ~ 1 | ACC, ~1 | species, ~1 | EF),
               R = list(species = cor), control = list(optimizer = "optim"),  mods = ~ plant_lifestage_2)
...
orchard_plot(phenolics_N,  mod = "Nfixing", group = "EF", data = phenolics, xlab = "Standardised mean difference", transfm = "none", angle = 0, k =FALSE)+
  scale_fill_manual(values = colours2) + 
  scale_colour_manual(values = colours2) +
    theme(axis.line = element_line(color='black'),
        plot.background = element_blank(),
